...
protected:
  ...
  OptionRule(Parser *parser, std::string long_name, bool is_flag = false, char short_name=0);
...
protected:
  ...
  std::list<std::string> _given_values;
...
public:
  ...
  RuleInterface &Flag(const std::string &name, char short_name=0) {
    RuleInterface *rule = new OptionRule(this, name, true, short_name);
    return *rule;
  }

  /** Constructs a @c OptionRule as an option. */
  RuleInterface &Option(const std::string &name, char short_name=0) {
    RuleInterface *rule = new OptionRule(this, name, false, short_name);
    return *rule;
  }

  /** Constructs a @c ValueRule. */
  RuleInterface &Value() {
    RuleInterface *rule = new ValueRule(this);
    return *rule;
  }

  /** Constructs a @c OptionalRule. */
  RuleInterface &opt(RuleInterface &rule) {
    RuleInterface *newrule = new OptionalRule(this, &rule);
    return *newrule;
  }

  /** Constructs a @c ZeroOrMoreRule. */
  RuleInterface &zeroOrMore(RuleInterface &rule) {

...
GillespieCLE::GillespieCLE(Ast::Model &model, int size, int seed) :
...

{
      ...
      for(int sid=0;sid<this->ensembleSize;sid++)
      {
          ...
          while(t < step)
          {
              ...
              noise = this->reduced_stoichiometry*(prop.cwiseSqrt()).asDiagonal()*this->normal(this->numReactions());
              drift = tau*this->reduced_stoichiometry*prop+sqrttau*noise;

              this->reducedState.row(sid) += (drift)+sqrt(tau)*noise;
